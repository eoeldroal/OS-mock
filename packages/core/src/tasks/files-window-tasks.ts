...
import { openExplorerWithFiles } from "./scenario-builders.js";
import {
  addBrowserWindow,
  addExplorerWindow,
  addFiles,
...
function companionWindows() {
  return {
    browserWindow: { windowId: "browser-main", bounds: BRW, focused: false, minimized: true },
    terminalWindow: { windowId: "terminal-main", bounds: TRM, focused: false, minimized: true },
    mailWindow: { windowId: "mail-main", bounds: MIL, focused: false, minimized: true }
  } as const;
}
...
function setupOpenFile(seed: number, vp: Viewport, dc: number) {
  ...
  const e = openExplorerWithFiles({
    instruction: `I need to check what's inside ${name}, could you open it?`,
    viewport: vp,
    files: rot([createFile("file-target", name, "File content"), ...dists(dc)], seed),
    explorerWindow: { windowId: "explorer-main", bounds: EXP, focused: true, minimized: false },
    ...companionWindows()
  });
...
function setupRename(seed: number, vp: Viewport, dc: number, presel: boolean) {
  ...
  const e = openExplorerWithFiles({
    instruction: instr,
    viewport: vp,
    files: rot([createFile("file-target", old, "Body"), ...dists(dc)], seed),
    explorerWindow: {
      windowId: "explorer-main",
      bounds: EXP,
      focused: true,
      minimized: false,
      selectedFileId: presel ? "file-target" : undefined
    },
    ...companionWindows()
  });
...
function setupOpenAppendSave(seed: number, vp: Viewport, dc: number) {
  ...
  const e = openExplorerWithFiles({
    instruction: `Open ${name}, add "${ap.trim()}" at the end, and save the file.`,
    viewport: vp,
    files: rot([createFile("file-target", name, init), ...dists(dc)], seed),
    explorerWindow: { windowId: "explorer-main", bounds: EXP, focused: true, minimized: false },
    ...companionWindows()
  });
...
function setupRenameThenOpen(seed: number, vp: Viewport, dc: number) {
  ...
  const nw = pick(seed, R_NEW);
  const e = openExplorerWithFiles({
    instruction: `Rename ${old} to ${nw}, then open it so I can start editing.`,
    viewport: vp,
    files: rot([createFile("file-target", old, "Body"), ...dists(dc)], seed),
    explorerWindow: { windowId: "explorer-main", bounds: EXP, focused: true, minimized: false },
    ...companionWindows()
...
function setupDockLaunchOpen(seed: number, vp: Viewport, dc: number) {
  const name = pick(seed, T_NAMES);
  const e = openExplorerWithFiles({
    instruction: `The file manager isn't running. Launch it from the dock and open ${name}.`,
    viewport: vp,
    files: rot([createFile("file-target", name, "File content"), ...dists(dc)], seed),
...
function setupDockLaunchRename(seed: number, vp: Viewport, dc: number) {
  ...
  const nw = pick(seed, R_NEW);
  const e = openExplorerWithFiles({
    instruction: `Files app is closed. Open it from the dock, then rename ${old} to ${nw}.`,
    viewport: vp,
    files: rot([createFile("file-target", old, "Body"), ...dists(dc)], seed),
    explorerWindow: false,
    ...companionWindows()
