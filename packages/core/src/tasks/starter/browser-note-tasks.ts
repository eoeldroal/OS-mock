...
function buildBrowserLogTaskIdSimple(seed: number, viewport: Viewport) {
  ...
  const envState = openBrowserNoteScenario({
    instruction: `In Firefox, select the ${v.label} category and "${v.task}" task, write its task id into ${v.fileName}, and save.`,
    viewport,
...
function buildBrowserHelpLogSummarySimple(seed: number, viewport: Viewport) {
  ...
  const v = variants[Math.abs(seed) % variants.length];
  const envState = openBrowserNoteScenario({
    instruction: `In Firefox, switch to Ubuntu help, write the requested help line into ${v.fileName}, and save.`,
    viewport,
    start: { page: "explorer", categoryId: "workflow", taskId: "workflow_mail_bridge" },
    browserBounds: browserBounds(),
    explorerWindow: {
...
function buildBrowserSelectFromHelpAndLogPreopen(seed: number, viewport: Viewport) {
  ...
  const v = variants[Math.abs(seed) % variants.length];
  const envState = openBrowserNoteScenario({
    instruction: `In Firefox, switch from Ubuntu help to Task Board, select the ${v.label} category and "${v.task}" task, write its task id into the open task-log.txt, and save.`,
    viewport,
    start: { page: "help", topicId: "dock-basics" },
    browserBounds: browserBounds(),
    explorerWindow: false,
    noteTarget: {
      fileId: "file-task-log",
      fileName: "task-log.txt",
      initialContent: "",
      preopen: true,
      windowId: "notes-task-log",
      bounds: noteBoundsRight(),
      focused: false,
      minimized: false
    }
...
export const STARTER_BROWSER_NOTE_TASKS: TaskSpec[] = [
  {
    ...
    summary: {
      ...
      level: "B",
      apps: ["browser", "note"],
      startState: "Firefox is focused on Task Board with the wrong selection, and a blank task-id note exists unopened in File Explorer.",
