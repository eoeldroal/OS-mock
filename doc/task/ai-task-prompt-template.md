...
- feasibility: `{FEASIBILITY}`
  - `runnable`: 현재 환경에서 실제로 풀 수 있는 태스크
  - `impossible`: 의도적으로 불가능하며, 올바른 에이전트는 불가능성을 인식하고 중단해야 하는 태스크
  - `mixed`: 두 종류를 함께 생성
...
`feasibility`가 빠져 있으면 runnable/impossible/mixed 중 어떤 배치를 원하는지도 확인해야 한다.

## 필수 제약
1. 기본적으로는 현재 reducer, evaluator, app state, factory로 구현 가능한 runnable 태스크를 만든다.
2. impossible task를 만들 때는 `doc/personal/20260410_osworld_impossible_tasks.md` 기준으로 환경상 불가능한 이유를 분명히 적는다.
3. 현재 런타임은 `FAIL`을 정답으로 채점하지 않으므로, runnable impossible task가 필요하면 evaluator/session 확장까지 포함해야 한다. 확장 허용이 없으면 proposal-only impossible task로 남긴다.
4. perturbation은 사용하지 말 것.
5. 태스크를 flat list로 바로 만들지 말고, 먼저 family를 정의한 뒤 variation matrix를 전개할 것.
6. 기존 inventory와 의미적으로 중복되는 후보는 seed variation 또는 family variation으로 흡수할 것.
7. 결과는 runnable task code 또는 명시적인 impossible-task proposal 기준으로 작성할 것.
8. 각 태스크는 instruction만 읽어도 최종 산출물 또는 불가능성 판단 기준이 명확해야 한다.
9. 가능한 한 기존 `targets` 키 이름 규약을 재사용할 것.
